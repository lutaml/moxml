...
known Ox ordering issues. The full test suite with all adapters runs in the `ox-rexml`
CI job; the `ruby-versions` CI job tests only Nokogiri and Oga adapters.

To run tests with a specific adapter set locally:

[source,bash]
----
# Nokogiri + Oga only (fast)
MOXML_ROUNDTRIP_ADAPTERS=nokogiri,oga bundle exec rake spec:consistency

# Ox + REXML only (slower)
MOXML_ROUNDTRIP_ADAPTERS=ox,rexml MOXML_ROUNDTRIP_TIMEOUT=300 bundle exec rake spec:consistency
----
