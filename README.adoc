...
Moxml supports `EntityReference` nodes for preserving entity syntax in XML documents. This enables round-trip preservation of entity references like `&nbsp;`, `&copy;`, and custom entities defined in the DOCTYPE.
...
[source,bash]
----
# Run round-trip tests (default: 10 fixtures)
bundle exec rake spec:roundtrip

# Run all fixtures (may be slow)
MOXML_ROUNDTRIP_MAX_FIXTURES=all bundle exec rake spec:roundtrip

# Run with a specific number of fixtures
MOXML_ROUNDTRIP_MAX_FIXTURES=50 bundle exec rake spec:roundtrip
----

The default fixture limit (10) ensures CI runs complete quickly while still
providing meaningful coverage. The full suite (920+ fixtures) can be used
for comprehensive local validation.
