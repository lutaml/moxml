...
- Keyword arguments on hot paths allocate a Hash per call on Ruby
  3.x — `Node.wrap` priming wrapper state via kwargs cost +1
  allocation per minted wrapper (exactly +601 on a 100-element
  mixed bench, every adapter). Positional optional arguments do
  not allocate; prefer them on mint/construction paths.
...
| Cold walk (parse + wrapper mint, 752 nodes) | 811µs | 720µs (-11%) |
