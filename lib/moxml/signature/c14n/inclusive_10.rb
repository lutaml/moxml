    ...
    module C14n
      ...
      class Inclusive10
        ...
        def render_element(element, writer, context, with_comments, is_apex)
          ...
          # At apex: use full in-scope set so inherited namespaces get URIs.
          # At descendants: the context's accumulated stack already reflects
          # the in-scope chain, so we use it as-is.
          ...
          # The set of namespaces to render on this element:
          #   apex  → ALL in-scope namespaces
          #   child → only those declared on this element whose URI differs
          #           from what's already rendered in the output ancestor.
